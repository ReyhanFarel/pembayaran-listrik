...
<div class="bg-white p-6 rounded-lg shadow-md w-full max-w-lg mx-auto">
    <h1 class="text-3xl font-bold text-gray-800 mb-6 text-center">Tambah Penggunaan Listrik Baru</h1>
    ...
    <form action="{{ Auth::guard('web')->user()->level_id == 1 ? route('admin.penggunaans.store') : route('petugas.penggunaans.store') }}" method="POST">
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
                ...
                <input type="number" id="tahun" name="tahun" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('tahun') border-red-500 @enderror" value="{{ old('tahun', date('Y')) }}" required min="2000" max="{{ date('Y') + 1 }}">
                @error('tahun')
                    <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="mb-4">
            ...
            <input type="number" id="meter_awal" name="meter_awal" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('meter_awal') border-red-500 @enderror" value="{{ old('meter_awal') }}" required min="0">
            @error('meter_awal')
                <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
            @enderror
        </div>
        <div class="mb-6">
            ...
            <input type="number" id="meter_akhir" name="meter_akhir" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('meter_akhir') border-red-500 @enderror" value="{{ old('meter_akhir') }}" required min="0">
            @error('meter_akhir')
                <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
            @enderror
        </div>
