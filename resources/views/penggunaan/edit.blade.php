...
@section('content')
    ...
    <div class="bg-white p-6 rounded-none border-4 border-neutral-900 neo-brutal-shadow-black w-full max-w-lg mx-auto">
        <h1 class="text-3xl font-extrabold text-neutral-900 mb-6 text-center uppercase">Edit Penggunaan Listrik</h1>
        ...
        <form
            action="{{ Auth::guard('web')->user()->level_id == 1 ? route('admin.penggunaans.update', $penggunaan->id) : route('petugas.penggunaans.update', $penggunaan->id) }}"
            method="POST">
            @csrf
            @method('PUT')
            <div class="mb-6">
                ...
                <select id="pelanggan_id" name="pelanggan_id"
                    ...
                    @foreach ($pelanggans as $pelanggan)
                        <option value="{{ $pelanggan->id }}"
                            {{ old('pelanggan_id', $penggunaan->pelanggan_id) == $pelanggan->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <select id="bulan" name="bulan"
                        ...
                        @foreach ($bulanNames as $bulanName)
                            <option value="{{ $bulanName }}"
                                {{ old('bulan', $penggunaan->bulan) == $bulanName ? 'selected' : '' }}>{{ $bulanName }}
                            </option>
                        @endforeach
                    </select>
                    @error('bulan')
                ...
                <div>
                    ...
                    <input type="number" id="tahun" name="tahun"
                        ...
                        value="{{ old('tahun', $penggunaan->tahun) }}" required min="2000" max="{{ date('Y') + 1 }}">
                    @error('tahun')
                        <p class="text-red-600 text-sm italic mt-2">{{ $message }}</p>
                    @enderror
                </div>
